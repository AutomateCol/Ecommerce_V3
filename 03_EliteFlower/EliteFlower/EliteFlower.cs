...
namespace EliteFlower
{
    public partial class EliteFlower : Form
    {
        ...
        bool flagPendientePulmon = false;
        bool flagPendienteSorter = false;
       ...
       // private ModbusClient modbusElevadores = new ModbusClient();
        ...
        private void EliteFlower_Load(object sender, EventArgs e)
        {
            ...
            btnEmergencyStop.Visible = false;
            ...
            ((Control)this.tabElite).Enabled = false;
                            ...
                            if (MessageBox.Show(UIMessages.EliteFlower(3, mnuELEnglish.Checked), UIMessages.EliteFlower(58, mnuELEnglish.Checked), MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
                            {
                                ...
                                if (CheckDB(distinctIDS, false))
                                {
                                    ...
                                    if (disparitystages == balancedStages)
                                    {
                                        ...
                                        (int, int, int, int, int) valuesPLC = Mongoose.GetValuesPLC();
                                        timeBlinkLeds = valuesPLC.Item1;
        ...
        private void serialStage21_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            try
            {
                ...
                if (workerUpForm)
                {
                    ...
                    if (matchTrack.Success)
                    {
                        DoActionSerial(serialStage12.PortName, 1, 0, matchTrack.Value, "A", true);
                    }
                    else
                    {
                        Match matchOrder = Regex.Match(entry, patternOrder);
                        if (matchOrder.Success)
                        {
                            DoActionSerial(serialStage12.PortName, 1, 0, matchOrder.Value, "A", false);
                        }
                    }
                }
            }
        ...
        private void serialPLC_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            try
            {
                ...
                if (response.Length < 4)
                {
                    ...
                    string[] charResponse = response.Select(x => new string(x, 1)).ToArray();
                    if (charResponse.Length > 0)
                    {
                        string responseChar = filterResponseSerialData(charResponse);
                        switch (responseChar)
                        {
                            case "A":
                                Console.WriteLine("------");
                                Console.WriteLine("PeticionesHandler - Automatico\n");
                                Console.WriteLine("------");
                                //-- Enviar peticiones de Sorter
                                bool[] _sendTaskSorter3 = modbusElevadores.ReadCoils(46, 1); //tarea del sorter
                                bool[] _sendTaskSorter4 = modbusElevadores.ReadCoils(47, 1); //tarea sorter-empujador
                                Console.WriteLine(_sendTaskSorter3[0]);
                                Console.WriteLine(_sendTaskSorter4[0]);
                                if (_sendTaskSorter3[0] == false && _sendTaskSorter4[0] == false)
                                {

                                    Console.WriteLine($"Peticiones Sorter");
                                    modbusElevadores.WriteSingleCoil(49, true);
                                    getNuevaPosSorter();                            //-- Manda el sorter a POS
                                }
                                Console.WriteLine("------");
                                //-- Enviar peticiones de Empujador
                                bool _sorterBusy = _sendTaskSorter3[0] || _sendTaskSorter4[0];
                                bool[] _sendTaskEmpujador = modbusElevadores.ReadCoils(48, 1);
                                Console.WriteLine(_sendTaskEmpujador[0]);
                                Console.WriteLine($"SorterBusy {_sorterBusy}");
                                if (_sendTaskEmpujador[0] == false)
                                {
                                    Console.WriteLine($"Peticiones Empujador");
                                    getNuevaPosEmpujador(_sorterBusy);              //-- Manda el empujador a POS
                                }
                                Console.WriteLine("------");
                                Console.WriteLine($"Empujador en pos {lastPositionEmpujador.Pos + 1}");
                                Console.WriteLine($"Sorter en pos {lastPositionSorter.Pos + 1}");
                                Console.WriteLine("------");
                                break;
                            case "B":
                                Console.WriteLine("PeticionesElevadores - Automatico\n");
                                actualizarPedidoCajaElevador();     //-- Pone en cola la tarea
                                actualizaInfoTextos();              //-- Actualiza textos
                                break;
                            case "C":
                                Console.WriteLine("Termino Empujador - Automatico\n");
                                if (llenarElevador.Count > 0)
                                {
                                    msgMesanin taskSorter = llenarElevador.Dequeue();           //-- Saca peticion terminada de la cola
                                    setColaMesanin(llenarPulmon, new List<string>() { taskSorter.Id }, new List<int>() { taskSorter.Pos });
                                    actualizarSCajaPulmon();
                                }
                                actualizaInfoTextos();              //-- Actualiza textos
                                break;
                            case "D":
                                Console.WriteLine("Termino Sorter - Automatico\n");
                                if (llenarPulmon.Count > 0)
                                {
                                    msgMesanin taskFinished = llenarPulmon.Dequeue();           //-- Saca peticion terminada de la cola
                                    lastPositionSorter.Pos = 4;
                                    actualizarSCajaPulmon();
                                    if (llenarPulmon.Count == 0)
                                    {
                                        modbusElevadores.WriteSingleCoil(49, false);
                                    }
                                }
                                actualizaInfoTextos();              //-- Actualiza textos
                                break;
                            case "E":
                                Console.WriteLine("Termino SorterEmpujador - Automatico\n");
                                if (llenarPulmon.Count > 0)
                                {
                                    msgMesanin taskFinished = llenarPulmon.Dequeue();           //-- Saca peticion terminada de la cola
                                    showNextBox2Fill();
                                ...
                                break;
                            case "F":
                                Console.WriteLine("Termino Error Empujador - Automatico\n");
                                if (llenarElevador.Count > 0)
                                {
                                    msgMesanin taskSorter = llenarElevador.Dequeue();           //-- Saca peticion terminada de la cola
                                }
                            ...
                            case "P":
                                ...
                                break;
                            //case "Z":
                            //    throw new PLCUndervoltageException();
                            //    break;
                            default:
                                //Console.WriteLine($"Data Slime {responseChar}");
                                break;
                        }
                    }
                }
                ...
                {
                    //Console.WriteLine($"NO ACCEPTED - {response}");
                }
                actualizaInfoTextos();
            }
            ...
            else
            {
...

                {
                    ...
                    if (dialogResult == DialogResult.Yes)
                    {
                        ...
                        txtControl.Text = "0.10";
                        oTask2.Start();
                        txtControl.Text = "0.11";
                        await oTask2;
                        txtControl.Text = "0.12";
                        HideLoader();
                        txtControl.Text = "0.13";
                        manBalanceCheck = true;
                        MessageBox.Show(UIMessages.EliteFlower(79, mnuELEnglish.Checked), "EliteFlower", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
        ...
        {
            UnbalanceDB_Data();
            txtControl.Text = "0.41";
            List<bool> WorkersChecked = new List<bool>() { chb_worker1.Checked, chb_worker2.Checked, chb_worker3.Checked };
            txtControl.Text = "0.42";
            int AvailableWorkers = WorkersChecked.Where(c => c).Count();
            txtControl.Text = "0.43";
            List<VaseCount> DataVase = Mongoose.GetDataVase("Data");
            txtControl.Text = "0.44";
        ...
        private int CalculateDelayAddon(int _CodedBoxPerHour)
        {
            ...
            if (_boxPerHourBandVel > 830)
            {
                return (int)((33 - 0.03 * 830) * 1000);
            }
            else if (_boxPerHourBandVel < 83)
            {
                return (int)((33 - 0.03 * 83) * 1000);
            }
            else
            {
                return (int)((33 - 0.03 * _boxPerHourBandVel) * 1000);
            }
        }
        ...
        private void btnInicio_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                //Console.WriteLine($"BalanceCheck {balanceCheck}");
                        ...
                        if (modbusElevadores.Available(2))
                        {
                            ...
                            if (initialFeed == false)
                            {
                                ...
                                if (llenadoInicial.Count > 0)
                                {
                                    ...
                                    //escribe en los registros de retencion 1000 y 1001, los valores 15. 
                                    // si el registro esta entre un valor de 1 a 9, este se encontraria asigando a un elevador
                                    modbusElevadores.WriteMultipleRegisters(REGS_ELEVADORES, new int[] { 15, 15 });
                                    modbusElevadores.WriteMultipleCoils(50, new bool[] { true, false });    //- InitialFill - Automatico
                                }
                                else
                                {
                                    //TODO
        ...
        private string filterResponseSerialData(string[] request)
        {
            string response = "";
            if (request.Length == 1)
            {
            ...
            {
                response = request.GroupBy(s => s).OrderByDescending(s => s.Count()).First().Key;
            }
            return response;
        }

        private void getNuevaPosSorter()
        {
            //MLlenado pulmon = getInfoMesanin();
            //setColaSorterEmpujador(pulmon);
            bool pendientesPulmones = llenarPulmon.Count > 0;

            if (pendientesPulmones) //-- Solo pendientes de pulmones
            {
                msgMesanin cajaPedida = llenarPulmon.Peek();
                textBox2.Text = cajaPedida.Pos.ToString();
                textBox3.Text = lastPositionEmpujador.Pos.ToString();
                textBox4.Text = llenarPulmon.Count.ToString();
                
                if (cajaPedida.Pos == lastPositionEmpujador.Pos || llenarPulmon.Count == 1 )
                {
                    msgMesanin nuevaPosEmpujador = moverPosicionAlternaEmpujador(cajaPedida, lastPositionEmpujador, false);
                    msgMesanin cajaPPulmon = llenarPulmon.Peek();
                    lastPositionSorter = cajaPPulmon;
                    lastPositionEmpujador = nuevaPosEmpujador;
                    Mongoose.SetLastPosZone2(new List<int>() { lastPositionSorter.Pos + 1, lastPositionEmpujador.Pos + 1 });
                    updateGraphicsMesaninSorter(lastPositionSorter.Pos + 1);
                    setPosicionServoMesanin(0, lastPositionSorter);
                    setPosicionServoMesanin(1, lastPositionEmpujador);
                    txtControlP.Text = "1";
                   
                    modbusElevadores.WriteSingleCoil(COILS_ELEVADORES + 47, true); //-- Empujador
                  

                    //Console.WriteLine("SorterAmbos - S");
                }
                else
                {
                    msgMesanin cajaPPulmon = llenarPulmon.Peek();
                    lastPositionSorter = cajaPPulmon;
                    Mongoose.SetLastPosZone2(new List<int>() { lastPositionSorter.Pos + 1, -1 });
                    updateGraphicsMesaninSorter(lastPositionSorter.Pos + 1);
                    setPosicionServoMesanin(0, lastPositionSorter);
                    txtControlP.Text = "2";
                    modbusElevadores.WriteSingleCoil(COILS_ELEVADORES + 46, true); //-- Empujador
                  
                    //Console.WriteLine("Sorter - S");
                }
            }
        }
        ...
        public MLlenado getInfoMesanin()
        {
            ...
            //MongoClient client = new MongoClient(settingsMongoDB);
            MongoClient client = new MongoClient(mongoDBConnection);
            IMongoDatabase database = client.GetDatabase("EliteFlower");
        ...
        private void setPosicionServoMesanin(int servo, msgMesanin caja)
        {
            try
            {
               
                modbusElevadores.WriteSingleRegister(REGS_ELEVADORES + servo, caja.Pos + 1);
                txtLastPos.Text = (caja.Pos).ToString();
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, "Error writing", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        private msgMesanin moverPosicionAlternaEmpujador(msgMesanin nuevaPosSorter, msgMesanin posEmpujador, bool elevadorExisteNuevaPos)
        {
            int posicionLlegadaEmpujador = 4;
            msgMesanin newPos = new msgMesanin();
            MLlenado pulmon = getInfoMesanin();
            List<int> posicionesPosibles = new List<int>();
            if (elevadorExisteNuevaPos)
            {
                posicionesPosibles = pulmon.Quantitys.Select((valor, posicion) => (valor == 1 && 1 <= Math.Abs(posicion - posEmpujador.Pos) && Math.Abs(posicion - nuevaPosSorter.Pos) <= 2) ? posicion : -1).Where(output => output >= 0).ToList();
                //newPos.Pos = (posicionesPosibles.Count == 0) ? posicionLlegadaEmpujador : posicionesPosibles[0];
                newPos.Pos = (posicionesPosibles.Count == 0) ? posicionLlegadaEmpujador : posicionesPosibles[posicionesPosibles.Count - 1];
                if (newPos.Pos == posEmpujador.Pos)
                {
                    //newPos.Pos = (newPos.Pos != 8) ? newPos.Pos++ : newPos.Pos--; //-- Se hace a la izquierda 
                    newPos.Pos = (newPos.Pos != 0) ? newPos.Pos-- : newPos.Pos++; //-- Se hace a la derecha 
                    return newPos;
                }
                else
                {
            ...
            {
                posicionesPosibles = pulmon.Quantitys.Select((valor, posicion) => (valor == 1 && 1 <= Math.Abs(posicion - nuevaPosSorter.Pos) && Math.Abs(posicion - nuevaPosSorter.Pos) <= 2) ? posicion : -1).Where(output => output >= 0).ToList();
                msgMesanin ultimaPosEmpujador = posEmpujador;
                newPos.Pos = (posicionesPosibles.Count == 0) ? posicionLlegadaEmpujador : posicionesPosibles[0];
                if (newPos.Pos == ultimaPosEmpujador.Pos)
                {
                    newPos.Pos++;
                    return newPos;
                }
        ...
        private void getNuevaPosEmpujador(bool _SorterBusy)
        {
            MLlenado pulmon = getInfoMesanin();
            //setColaSorterEmpujador(pulmon);
            bool pendientesElevadores = llenarElevador.Count > 0;

            //se establecen las zonas para el movimiento del sorter y el empujador
            if (pendientesElevadores) //-- Solo pendientes de elevadores
            {
                List<int> cajasDisponibles = pulmon.Quantitys.Select((valor, posicion) => (valor == 1) ? posicion : -1).Where(output => output >= 0).ToList();
                msgMesanin cajaPedida = llenarElevador.Peek(); // toma el primer valor en la lista del queue llenar elevador
                if (cajasDisponibles.Contains(cajaPedida.Pos))
                {
                    msgMesanin cajaAElevador = llenarElevador.Peek();
                    if (_SorterBusy == true)
                    {
                        List<int> zone1 = _ElevadoresMesanin.GetRange(0, lastPositionSorter.Pos);
                        List<int> zone2 = _ElevadoresMesanin.GetRange(lastPositionSorter.Pos + 1, 8 - lastPositionSorter.Pos);
                        //msgMesanin cajaAElevador = llenarElevador.Dequeue();
                        if (zone1.Contains(lastPositionEmpujador.Pos) && zone1.Contains(cajaAElevador.Pos))
                        {
                            //setMoverMesanin(lastPositionSorter, cajaAElevador, false, 48); //-- Empujador
                            lastPositionEmpujador = cajaAElevador;
                            Mongoose.SetLastPosZone2(new List<int>() { -1, lastPositionEmpujador.Pos + 1 });
                            setPosicionServoMesanin(1, lastPositionEmpujador);
                            txtControlP.Text = "3";
                            modbusElevadores.WriteSingleCoil(COILS_ELEVADORES + 48, true); //-- Empujador
                            //Console.WriteLine("Empujador - E");
                        }
                        else if (zone2.Contains(lastPositionEmpujador.Pos) && zone2.Contains(cajaAElevador.Pos))
                        {
                            //setMoverMesanin(lastPositionSorter, cajaAElevador, false, 48); //-- Empujador
                            lastPositionEmpujador = cajaAElevador;
                            Mongoose.SetLastPosZone2(new List<int>() { -1, lastPositionEmpujador.Pos + 1 });
                        ...
                        {
                            msgMesanin cambiarCajaPedida = llenarElevador.Dequeue();
                            setMoverCajaCola(cambiarCajaPedida); //-- IDLE
                            setColaMesanin(llenarPulmon, new List<string>() { cambiarCajaPedida.Id }, new List<int>() { cambiarCajaPedida.Pos });
                            actualizarSCajaPulmon();
                            txtControlP.Text = "5";
                            //Console.WriteLine("IDLE - RT1");
                        }
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    //Console.WriteLine("IDLE - RT2");
                }
            }
        }

        private void setMoverCajaCola(msgMesanin cambiarCajaPedida)
        {
            List<string> vases = new List<string>() { cambiarCajaPedida.Id };
            List<int> posiciones = new List<int>() { cambiarCajaPedida.Pos };
            setColaMesanin(llenarElevador, vases, posiciones);
            //setColaMesanin(llenarPulmon, vases, posiciones);
        }

        private void actualizarPedidoCajaPulmon()
        {
            try
            {
                int[] tt = modbusElevadores.ReadHoldingRegisters(1002, 1); // lee los pulmones que estan vacios
                string s = Convert.ToString(tt[0], 2); //Convert to binary in a string
                int[] _valuesSPulmones = s.PadLeft(9, '0') // Add 0's from left
                             .Select(c => int.Parse(c.ToString())) // convert each char to int
                             .Reverse()  // Reverse list
                             .ToArray(); // Convert IEnumerable from select to Array
                List<int> emptyLungs = new List<int>();
                List<bool> enableStages = new List<bool>() { 
                    chb_worker1.Checked, chb_worker1.Checked, chb_worker1.Checked,
                    chb_worker2.Checked, chb_worker2.Checked, chb_worker2.Checked,
                    chb_worker3.Checked, chb_worker3.Checked, chb_worker3.Checked
                };
                for (int kk = 0; kk < 9; kk++)
                {
                    if (enableStages[kk] == true && _valuesSPulmones[kk] == 1)
                    {
                        emptyLungs.Add(kk);
                    }
                }
                //List<int> emptyLungs = _valuesSPulmones.Select((value, index) => value ? -1 : index).Where(o => o >= 0).ToList();
                if (emptyLungs.Count > 0)
                {
                    MLlenado llenado = getInfoMesanin();
                    List<string> vases = new List<string>();
                    for (int i = 0; i < emptyLungs.Count; i++)
                    {
                        vases.Add(llenado.References[emptyLungs[i]]);
                    }
                    setColaMesanin(llenarPulmon, vases, emptyLungs);
                }
            }
            catch (Exception ex)
            {
        ...
        private void actualizarPedidoCajaElevador()
        {
            try
            {
                bool[] _valuesSElevadores = modbusElevadores.ReadCoils(67, 9);
                updateGraphicsMesaninElevadores(_valuesSElevadores);
                List<int> emptyElevators = _valuesSElevadores.Select((value, index) => value ? index : -1).Where(o => o >= 0).ToList();
                if (emptyElevators.Count > 0)
                {
                    MLlenado llenado = getInfoMesanin();
                    List<string> vases = new List<string>();
                    for (int i = 0; i < emptyElevators.Count; i++)
                    {
                        vases.Add(llenado.References[emptyElevators[i]]);
                    }
                    setColaMesanin(llenarElevador, vases, emptyElevators);
                }
            }
            catch (Exception ex)
            {
        ...
        private void actualizarSCajaPulmon()
        {
            try
            {
                bool[] _valuesSPulmones = modbusElevadores.ReadCoils(58, 9);
                int[] emptyBoxes = _valuesSPulmones.Select((value, index) => value ? 0 : 1).ToArray();
                putActualizarPulmones(emptyBoxes);
                updateGraphicsMesaninPulmones(_valuesSPulmones);
                updateGraphicsBaseSorter();
            }
            catch (Exception ex)
        ...
        private void putActualizarPulmones(int[] boxInSlung)
        {
            //MongoClient client = new MongoClient(settingsMongoDB);
            MongoClient client = new MongoClient(mongoDBConnection);
            IMongoDatabase database = client.GetDatabase("EliteFlower");
            ...
            tt[0].Quantitys = boxInSlung;
            MReferenceDB.ReplaceOne(r => r.ID == tt[0].ID, tt[0]);
        }
