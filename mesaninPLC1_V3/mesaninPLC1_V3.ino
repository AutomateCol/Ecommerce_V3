...
#define CoilServoOn             91 //--COIL para indicar que los servos están en ON
...
bool flagServoOn = false; //Bandera que indica el estado ON/OFF de los servos
  ...
  */
  if (EMGFlag) {
    ...
    if (MB_C[servosSON])
    {
      ...
      P1.writeDiscrete(stateActuadores, SLDELEVADOR4);
      if (flagServoOn){
        modbusTCPServer.coilWrite(CoilServoOn, LOW);
        flagServoOn = false;
        _PL("Se Apagan los Servos");
      }
      else if (!flagServoOn){
        modbusTCPServer.coilWrite(CoilServoOn, HIGH);
        flagServoOn = true;
        _PL("Se Encienden los Servos");
      }
