...
#define CoilExit                91 //-- COIL para saber cuando el usuario se salió de la aplicación
...
void  offservo_exit() {
  ...
  if (modbusTCPServer.coilRead(CoilExit)) {
    if (flagServoON) {
      _PL("Apagando Servos");
      stateActuadores = stateActuadores ^ maskSON;
      P1.writeDiscrete(stateActuadores, SLDELEVADOR4);
      flagServoON = false;
      modbusTCPServer.coilWrite(CoilExit, LOW);
    }
    else {
      _PL("Servos Apagados");
    }
