...
Queue<Elevador> queueStage1 = Queue<Elevador>(3);
Queue<Elevador> queueStage2 = Queue<Elevador>(3);
Queue<Elevador> queueStage3 = Queue<Elevador>(3);
Queue<int> queueElevadores = Queue<int>(9);

struct lastTask {
  int pos;
  bool finished;
};
lastTask _TaskEmpujador = { -1, true };
lastTask _TaskSorter = { -1, true };
lastTask _TaskSorterEmpujador = { -1, true };
...
//! SLOT 5
#define maskDIBO                0x01FF  //-- Mascara pasa sacar los datos si los pulmones se encuentran llenos o no
#define maskCSorter             0x0200  //-- Mascara para sacar informacion de la presencia de la caja en el sorter
#define maskMPST1               0x0C00  //-- Mascara pasa sacar el dato donde se encuentra el piston del elevador
#define maskMPST2               0x3000  //-- Mascara pasa sacar el dato donde se encuentra el piston del empujador
//! SLOT 6
#define maskDIDO                0x01FF  //-- Mascara pasa sacar los datos si los elevadores tienen caja o no incluyendo el elevador del mesanin
#define maskSHOME               0x3C00  //-- Mascara para obtener el dato si las estaciones hicieron el home
#define maskSALRM               0xC000  //-- Mascara para obtener el dato si las estaciones se encuentran alarmadas
//! SLOT 7
#define maskCMDOK               0x0FFF  //-- Mascara para sacar los datos si los elevadores terminaron de hacer la ultima posicion a las que se le envia
#define maskSRDY                0xF000  //-- Mascara para obtener el dato si las estaciones estan listas luego de hacer Servo ON
//! OTROS
#define maskINV                 0x01FF  //-- Mascara para invertir la salida de los sensores de los elevadores
...
void getListQueue(Queue<Elevador> queue, int positions[])
{
  /*
      Rellena en una lista las posiciones con la cantidad de elementos
      que tiene en la cola de cada estacion.
  */

  Queue<Elevador> cpQueue = queue;
  positions[0] = -1;
  positions[1] = -1;
  positions[2] = -1;
  for (int ii = 0; ii < 3; ii = ii + 1)
  {
    if (cpQueue.count() > 0)
    {
      Elevador item = cpQueue.pop();
      positions[ii] = item.pos;
    }
  }
}
...
void LeerSensoresNeumaticos() {
  ...
  {
    //    Serial.print("Sensor actuadores   ");
    //    Serial.print(ii + 1);
    //    Serial.print(" = ");
    //    Serial.println(_valuesSPistones[ii]);
...
void TriggerBandaSorter() {
  ...
  if (P1.readDiscrete(ENTELEVADOR3, 11) == 1) {
    stateActuadores = stateActuadores ^ maskMot;
    P1.writeDiscrete(stateActuadores, SLDELEVADOR4);
    if (BandON) {
      _PL("Banda Sorter OFF");
      BandON = false;
    }
    else {
      _PL("Banda Sorter ON");
